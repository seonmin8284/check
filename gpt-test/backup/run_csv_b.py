"""A/B 테스트 B안: 근거 추론 기반 목적 분할 + 의도 추론 + 경계 케이스 few-shot.

A안(run_csv.py) 대비 변경점만:
  1. 목적 분할을 표면 연결어가 아니라 "답하는 데 필요한 근거"에서 유도한다.
  2. 각 goal 에 의도분류표(intent_taxonomy.csv) 라벨을 붙인다.
  3. 경계 케이스 few-shot 8건을 프롬프트에 넣는다.
스키마의 나머지·모델·파라미터·출력 컬럼은 A안과 동일하게 두어 비교 가능성을 유지한다.

실행:
    .venv/Scripts/python.exe run_csv_b.py --input work.csv   --output work_out_b.csv
...
1. goals — decomposed, classified, and labeled with an intent
...
Assign exactly one `intent` per goal, from the taxonomy below. The intent
names *what the user is trying to get out of this goal in business terms*.
It is a second, independent read of the same goal — derive it from the
goal's target and the user's purpose, not mechanically from the facet you
just assigned. Where they genuinely disagree, leave them disagreeing; the
disagreement is signal.

I1-1  기본적인 금융 지식 > 금융 개념 및 용어
I1-2  기본적인 금융 지식 > 금융 상품 설명       (연금저축·IRP·ISA 등 상품 자체)
I2-1  규정 및 지침 > 세금 관련 정보
I2-2  규정 및 지침 > 법적 문제 및 규제          (서류 요건, 실명확인, 한도규제)
I2-3  규정 및 지침 > 투자 가이드                (유의사항, 권장 절차)
I3-1  사용 매뉴얼 > 티레이더M 사용법            (앱 화면·메뉴 경로, 조회 방법)
I3-2  사용 매뉴얼 > 투자 절차 안내              (개설·해지·재등록 등 업무 처리)
I4-1  기업 상장 및 IPO > IPO 기본 정보
I4-2  기업 상장 및 IPO > IPO 일정 및 절차
I5-1  시장 동향 및 분석 > 시장 전망             (지수·시장 전반)
I5-2  시장 동향 및 분석 > 산업 동향 분석        (업종·테마 업황)
I6-1  기업정보 > 기본 정보
I6-2  기업정보 > 기업 개요
I6-3  기업정보 > 뉴스
I6-4  기업정보 > 공시 및 주요 일정
I6-5  기업정보 > 배당 정보
I6-6  기업정보 > 재무 정보                      (확정 실적)
I6-7  기업정보 > 기업 분석                      (실적 원인·사업 영향 해석)
I6-8  기업정보 > 기업 평가 및 전망              (향후 실적·주가 전망, 평가 점수)
I7-1  시세 정보 > 주가 및 거래내역              (현재가, 거래량, 수급, 공매도)
I7-2  시세 정보 > 과거 시세 분석                (기간 추이, 52주 고저, 변동성)
I8-1  평가 및 밸류에이션 > 멀티플 정보
I8-2  평가 및 밸류에이션 > 컨센서스             (추정치, 목표주가)
I9-1  기타 > 기타 질의                          (환율 등)

Guidance on the intents that get confused:
- 확정 실적 조회는 I6-6, 아직 오지 않은 실적 추정치는 I8-2.
- 판단·전망 goal(type=assessment, facet=none)은 그 판단의 대상이 개별 기업
  이면 I6-8, 시장·업종이면 I5-1/I5-2.
- 절차를 묻되 그 절차의 근거가 법정 요건이면 goal 을 나누고, 절차 goal 은
  I3-2, 요건 goal 은 I2-2.
- 앱 어디서 보는지가 핵심이면 I3-1, 무엇을 해야 하는지가 핵심이면 I3-2.
...
# ─────────────────────────────────────────────────────────────
# SCHEMA — A안과 동일하되 goal 에 intent 1개만 추가
# ─────────────────────────────────────────────────────────────
...
INTENTS = [
    ...
    "I4-1", "I4-2",
    "I5-1", "I5-2",
    "I6-1", "I6-2", "I6-3", "I6-4", "I6-5", "I6-6", "I6-7", "I6-8",
    "I7-1", "I7-2",
    "I8-1", "I8-2",
    "I9-1",
]
            ...
            "items": _obj(
                {
                    ...
                    "intent": {
                        "type": "string",
                        "enum": INTENTS,
                        "description": "의도분류표 코드. facet 에서 기계적으로 유도하지 말 것",
                    },
...
def flatten(data: dict) -> dict:
    ...
    return {
        ...
        "intents": "\n".join(f"{g['id']} {g['intent']}" for g in goals),
        "entities": "\n".join(
...
def main() -> int:
    ...
    p.add_argument("--output", default="work_out_b.csv")
