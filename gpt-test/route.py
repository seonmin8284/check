...
# 되살릴 자리가 된다.
#
# ── 이 자리가 다시 열리는 조건 (2026-09-02, edge_audit.py) ──
#
# 폐기·되살림을 손으로 판단할 일이 아니다. 간선의 **한계 정밀도** q_m (그
# 간선이 유일 지지자일 때의 정답률) 을 손익분기 τ* = F_β/(1+β²) 과 비교하면
# 된다. τ* 는 β 의 함수이므로 지표를 바꾸면 간선 집합이 바뀐다.
#
#     β      τ*    τ* 미달 간선
#   0.5   0.594    10개
#   1.0   0.370     4개  ← themed issuer→news channel → get_stock_news (q_m 0.234)
#   1.5   0.228    (없음)
#   2.0   0.148    (없음)   ← 현행
#   3.0   0.074    (없음)
#
# F1 에서 죽는 4개에 themed issuer→news channel 의 재현율 간선이 들어 있다.
# 그 규칙이 F1 시절 폐기됐다가 F2 에서 되살아난 것은 우연이 아니었다는 뜻이고,
# 위 경위가 계산으로 재유도된다.
#
# β ≥ 1.5 에서 간선 집합은 불변이다. 따라서 이 자리가 다시 열리는 유일한
# 조건은 **지표를 F1 이하로 되돌리는 결정**이다. 그 전에는 손대지 않는다.
#
# 단, τ* 는 "이 호출 하나를 붙일까"의 **한계 판정**이지 간선 삭제 기준이 아니다.
# q_m 은 그 간선의 이질적인 발화들을 평균낸 값이라, 통째로 지우면 τ* 를 넘는
# 발화까지 함께 죽는다. 실제로 τ* 미달 간선을 끊어보면 어느 β 에서도 합집합에
# 진다 — β=1.0 에서 F1 0.7629 -> 0.7554. 그러므로 위 표는 "F1 로 되돌리면 4개를
# 뺀다"가 아니라 "4개를 다시 검토할 자격이 생긴다"로 읽어야 한다.
